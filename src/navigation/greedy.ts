...
export function runGreedy(
  ...
  _getCost: (from: number, to: number, penalty: number) => number,
  startId: number,
  endId: number,
  _slopePenalty: number
): THREE.Vector3[] {
  const openSet = new Set<number>([startId])
  const cameFrom = new Map<number, number>()
  const fScore = new Map<number, number>()

  vertices.forEach((_, i) => fScore.set(i, Infinity))
  ...
  while (openSet.size > 0) {
    ...
    for (const neighbor of getNeighbors(current)) {
      ...
      if (h >= (fScore.get(neighbor) ?? Infinity)) continue
      cameFrom.set(neighbor, current)
      fScore.set(neighbor, h)
